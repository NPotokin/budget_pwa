...
import { deleteCategory, fetchCategories, updateCategoryLimit, updateCategoryName } from "@/store/categories/categories.Thunk";

const EditCategory: React.FC = () => {
  const { categoryId } = useParams();
  const navigate = useNavigate();
  const dispatch = useAppDispatch();
  const category = useTypedSelector(state => state.categories.categories.find(category => category.id === categoryId) as Category);

  const [isEditingName, setIsEditingName] = useState(false);
  const [isEditingLimit, setIsEditingLimit] = useState(false);

  const nameForm = useForm<{ name: string }>({
    resolver: zodResolver(z.object({ name: z.string().min(2, "Category name must be at least 2 characters.") })),
            ...
            <form onSubmit={limitForm.handleSubmit(handleUpdateLimit)} className="flex flex-col space-y-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      <Input className="w-[90vw] mx-0" type="number" placeholder="Enter new Limit" {...field} />
                    </FormControl>
                  </FormItem>
                )}
              />
              <div className="flex space-x-4">
                <Button type="submit">Save</Button>
                <Button variant="outline" onClick={() => setIsEditingLimit(false)}>Cancel</Button>
              </div>
            </form>
          </Form>
        ) : (
          <Button className="justify-start px-0" variant="link" onClick={() => setIsEditingLimit(true)}>Change Limit</Button>
        )}
      </div>
      <Button 
        type={'button'} 
