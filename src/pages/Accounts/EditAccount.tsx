...
import { fetchAccounts, updateAccountName, updateAccountBalance, deleteAccount } from "@/store/accounts/accounts.Thunk";
...
const EditAccount: React.FC = () => {
  ...
  const account = useTypedSelector(state => state.accounts.list.find(account => account.id === accountId) as Account);

  const [isEditingName, setIsEditingName] = useState(false);
  const [isEditingBalance, setIsEditingBalance] = useState(false);

  const nameForm = useForm<{ name: string }>({
    resolver: zodResolver(z.object({ name: z.string().min(2, "Account name must be at least 2 characters.") })),
  ...
  useEffect(() => {
    if (!account) dispatch(fetchAccounts());
  }, [account, dispatch]);
    ...
    <div className="flex flex-col space-y-4">
      ...
      <div className="flex flex-col mx-4">
        {isEditingName ? (
          ...
          <Button className="justify-start px-0" variant="link" onClick={() => setIsEditingName(true)}>Change Name</Button>
        )}
      </div>
      ...
      <div className="flex flex-col mx-4">
        {isEditingBalance ? (
          <Form {...balanceForm}>
            <form onSubmit={balanceForm.handleSubmit(handleUpdateBalance)} className="flex flex-col space-y-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      <Input className="w-[90vw] mx-0" type="number" placeholder="Enter new balance" {...field} />
                    </FormControl>
                  </FormItem>
                )}
              />
          ...
          <Button className="justify-start px-0" variant="link" onClick={() => setIsEditingBalance(true)}>Change Balance</Button>
        )}
      </div>
      <Button 
        type={'button'} 
